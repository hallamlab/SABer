...
def run_minhash_recruiter(sig_path, mhr_path, sag_subcontigs, mg_subcontigs):
    ...
    if os.path.isfile(os.path.join(sig_path, mg_id + '.metaG.sig')):  # TODO: MG should only be loaded if required
        logging.info('[SABer]: Loading %s Signatures' % mg_id)
        mg_sig_list = sourmash.signature.load_signatures(os.path.join(sig_path, mg_id + \
                                                                      '.metaG.sig')
                                                         )
    else:
        ...
        with open(os.path.join(sig_path, mg_id + '.metaG.sig'), 'w') as mg_out:
    ...
    for sag_id, sag_headers, sag_subs in sag_subcontigs:
        if os.path.isfile(os.path.join(mhr_path, sag_id + '.mhr_recruits.tsv')):
            logging.info('[SABer]: Loading %s and MetaG signature recruit list\n' % sag_id)
            with open(os.path.join(mhr_path, sag_id + '.mhr_recruits.tsv'), 'r') as mhr_in:
                pass_list = [x.rstrip('\n').split('\t') for x in mhr_in.readlines()]
        else:
            # Calculate\Load MinHash Signatures with SourMash for SAG subseqs
            if os.path.isfile(os.path.join(sig_path, sag_id + '.SAG.sig')):
                print('[SABer]: Loading Signature for %s\n' % sag_id)
                sag_sig = sourmash.signature.load_one_signature(os.path.join(sig_path,
                                                                     sag_id + '.SAG.sig')
                                                                )
            else:
                ...
                with open(os.path.join(sig_path, sag_id + '.SAG.sig'), 'w') as sags_out:
            ...
            with open(os.path.join(mhr_path, sag_id + '.mhr_recruits.tsv'), 'w') as mhr_out:
                mhr_out.write('\n'.join(['\t'.join(x) for x in pass_list]))
        minhash_pass_list.extend(pass_list)
        print('[SABer]: MinHash recruited %s subcontigs to %s\n' % (str(len(minhash_pass_list)), sag_id))
